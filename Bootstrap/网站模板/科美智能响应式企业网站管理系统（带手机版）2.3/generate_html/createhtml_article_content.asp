...
'=================================================
function Run_createhtml_article_content(contentId)
...
do while not conRs.eof
	...
	if trim(a2_c_folder)&""="" then
		...
		folderPath=CreateContentFolder(fso,a2_b_folder,"")
		category_position="<a href='../"&a2_b_folder&"'>"&a2_b_name&"</a>"
		this_page_channel_name="_"&a2_b_name'设置本页title频道名称
		b_websiteroot="../"
		juhaoyong_mp_folder_file_name=a2_b_folder&"/"&a2_html_file_name
	else
		'调用文件夹检测和创建函数，创建文件夹，并返回当前栏目的物理文件夹路径（此文件夹只为生成静态html文件用，前台显示路径不用这个）
		folderPath=CreateContentFolder(fso,a2_b_folder,a2_c_folder)
		...
		b_websiteroot="../../"
		juhaoyong_mp_folder_file_name=a2_b_folder&"/"&a2_c_folder&"/"&a2_html_file_name
	end if
	...
	if a2_Files&""<>"" then
		...
		for df_ii=0 to df_all
			...
			articleDownloads=articleDownloads&"<a href='"&b_websiteroot&"upAttachFile/"&downloadFiles(df_ii)&"' target='_blank'>"&downloadFiles(df_ii)&"</a> <span class='ListDate'>(点击右键可另存)</span></div> "
		next
	end if
	...
	articleContent=articleContent&"<div class='title'><h3>"&a2_title&"</h3></div>"
	
	articleContent=articleContent&"<div class='infos'>"&GetFormatTime(a2_edit_time)&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击：<script language='javascript' src='"&b_websiteroot&"inc/click_number.asp?id="&a2_id&"'></script></div>"
	
	articleContent=articleContent&"<div class='maincontent clearfix'>"
	articleContent=articleContent&a2_content
	...
	this_page_banner="<div class='insidebanner'><a href='"&trim(a2_c_jhy_fd_image_url)&"'><img src='"&b_websiteroot&"css/"&SITE_STYLE_CSS_FOLDER&"/"&trim(a2_c_jhy_fd_image)&"' alt='"&trim(a2_c_jhy_fd_image_alt)&"'></a></div>"
	elseif trim(a2_b_jhy_fd_image)&""<>"" then
	this_page_banner="<div class='insidebanner'><a href='"&trim(a2_b_jhy_fd_image_url)&"'><img src='"&b_websiteroot&"css/"&SITE_STYLE_CSS_FOLDER&"/"&trim(a2_b_jhy_fd_image)&"' alt='"&trim(a2_b_jhy_fd_image_alt)&"'></a></div>"
	else
	this_page_banner=""
	end if
	...
	'最终输出写入生成文件
	if trim(a2_html_file_name&"")<>"" then'这个判断是为了让第一条内容不生成（因为第一条内容是空的，从第二条开始生成，且最后加了一次循环）
