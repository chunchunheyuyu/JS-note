...
if request("action")="AllDel" then
	...
	if Num=0 then
		response.Write JuhaoyongReturnHistoryPage("数据库在线备份","请选择要删除的文件")
		response.end
	end if
	
	if delconfirm<>"1" then
		response.Write"<table cellpadding=3 cellspacing=1 border=0 class=tableBorder align=center>"
		response.Write"	<tr><th class='tableHeaderText'>数据库在线备份</th></tr>"
	...
	if delconfirm<>"1" then
		response.Write JuhaoyongConfirmPage("数据库在线备份","删除后不可恢复，确定删除吗？","db_backup_del.asp?delconfirm=1&file_path_name="&file_path_name)
		response.end
	else
...
response.Write JuhaoyongResultPage("数据库在线备份","删除成功","db_backup_list.asp")
response.end
%>
