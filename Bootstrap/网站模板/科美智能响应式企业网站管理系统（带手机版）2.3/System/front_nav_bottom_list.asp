...
if action1="edit" then
	if isnumeric(order1)=false then
		response.Write JuhaoyongReturnHistoryPage("底部导航排序","排序值必须为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("底部导航排序","修改成功","front_nav_bottom_list.asp")
		response.end
	end if
end if
...
%>
	...
	<tr><th class="tableHeaderText">底部导航</th></tr>
...
<p>1、<font color="#009933" size="+1"><strong>增加、修改、删除导航后，请重新生成所有静态，然后到前台&nbsp;<font color="#FF0000">按F5刷新</font>&nbsp;查看效果。</strong></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="html_generate_alert.asp?htmltype=all"><span style=" font-weight:bold; color:#093; font-size:18px; text-decoration:none;"> >>>点击这里，去生成所有静态</span></a></p><p>2、一般情况，底部导航不需要修改。</font></p>
            </td>
          </tr>
          <tr>
            <td height="10" ></td>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="30"> <a href="front_nav_bottom_add.asp">添加新的底部导航</a></td>
          </tr>
          <tr>
            <td height="10"></td>
          </tr>
...
s_sql="select * from juhaoyong_tb_navfirst where fd_navfirst_type=2 order by  [order],id"
...
class_style="jhyabTabletdBgcolor01"
          ...
          <tr >
            ...
            <div align="center"><a href="front_nav_bottom_edit.asp?id=<%=rs("id")%>">修改</a> | <a href="front_nav_bottom_del.asp?id=<%=rs("id")%>">删除</a></div></td>
          </tr></form>
		  		  <%
		  rs.movenext
