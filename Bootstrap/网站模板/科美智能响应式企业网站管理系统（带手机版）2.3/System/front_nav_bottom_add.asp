...
If act="save" Then 
	...
	'检测名称不能为空
	if l_name&""="" then
		response.Write JuhaoyongReturnHistoryPage("底部导航添加","名称不能为空")
		response.end
	'检测排序必须是数字
	elseif isnumeric(l_order)=false then
		response.Write JuhaoyongReturnHistoryPage("底部导航添加","排序值只能为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("底部导航添加","添加成功","front_nav_bottom_list.asp")
		response.end
	end if
else
%>
	...
	<tr>
	  <th class="tableHeaderText">添加底部导航</th>
	</tr>
