...
if action1="edit" then
	if isnumeric(order1)=false then
		response.Write JuhaoyongReturnHistoryPage("一级导航排序","排序值必须为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("一级导航排序","修改成功","front_nav_first_list.asp")
		response.end
	end if
end if
...
%>
	...
	<tr><th class="tableHeaderText">顶部一级导航</th></tr>
...
<p>2、顶部一级导航最多设置8个。</p>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="30"> <a href="front_nav_first_add.asp">添加新的一级导航</a></td>
          </tr>
          <tr>
            <td height="10"></td>
          </tr>
...
s_sql="select * from juhaoyong_tb_navfirst where fd_navfirst_type=1 order by  [order],id"
...
class_style="jhyabTabletdBgcolor01"
          ...
          <tr >
            ...
            <div align="center"><a href="front_nav_first_edit.asp?id=<%=rs("id")%>">修改</a> | <a href="front_nav_first_del.asp?id=<%=rs("id")%>">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
