...
If act1="save" Then
	...
	'检测名称不能为空
	if l_name&""="" then
		response.Write JuhaoyongReturnHistoryPage("顶部二级导航修改","名称不能为空")
		response.end
	'分类选择检测
	elseif l_position&""="" then
		response.Write JuhaoyongReturnHistoryPage("顶部二级导航修改","请选择所属一级分类")
		response.end
	'检测排序必须是数字
	elseif isnumeric(l_order)=false then
		response.Write JuhaoyongReturnHistoryPage("顶部二级导航修改","排序值只能为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("顶部二级导航修改","修改成功","front_nav_second_list.asp?oneid="&oneid&"&act="&act&"&keywords="&keywords)
		response.end
	end if
else
%>
...
<%
	...
	<tr>
	  <th class="tableHeaderText">顶部二级导航修改</th>
	</tr>
