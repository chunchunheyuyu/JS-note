...
If act="save" Then 
	...
	'检测名称不能为空
	if l_name&""="" then
		response.Write JuhaoyongReturnHistoryPage("顶部一级导航添加","名称不能为空")
		response.end
	'检测排序必须是数字
	elseif isnumeric(l_order)=false then
		response.Write JuhaoyongReturnHistoryPage("顶部一级导航添加","排序值只能为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("顶部一级导航添加","添加成功","front_nav_first_list.asp")
		response.end
	end if
else
%>
	...
	<tr>
	  <th class="tableHeaderText">添加顶部一级导航</th>
	</tr>
	...
	<td width="85%" class="jhyabTabletdBgcolor01"><input name='name' type='text' id='name' size='70' maxlength="30">
	  ...
	  <td class="jhyabTabletdBgcolor01"><input name='order' type='text' value="10" size="3" maxlength="3">
只能是数字，数字越小排名越靠前</td>
	  </tr>
