...
If act1="save" Then 
	...
	'检测名称不能为空
	if l_name&""="" then
		response.Write JuhaoyongReturnHistoryPage("顶部一级导航修改","名称不能为空")
		response.end
	'检测排序必须是数字
	elseif isnumeric(l_order)=false then
		response.Write JuhaoyongReturnHistoryPage("顶部一级导航修改","排序值只能为数字")
		response.end
	else
		...
		response.Write JuhaoyongResultPage("顶部一级导航修改","修改成功","front_nav_first_list.asp")
		response.end
	end if
else
%>
...
<%
	set rs=server.createobject("adodb.recordset")
	sql="select * from juhaoyong_tb_navfirst where id="&article_id&""
	rs.open(sql),cn,1,1
	if not rs.eof and not rs.bof then%>
	<form id="form1" name="form1" method="post" action="?act1=save">
	<table cellpadding='3' cellspacing='1' border='0' class='tableBorder' align=center>
	<tr>
	  <th class="tableHeaderText">修改顶部一级导航</th>
	</tr>
	...
	<tr>
	  ...
	  <td class="jhyabTabletdBgcolor01"><input name='order' type='text' size="3" maxlength="3" value="<%=rs("order")%>">
只能是数字，数字越小排名越靠前</td>
	</tr>

	<tr>
		<td height="50" colspan=2  class="jhyabTabletdBgcolor02"><div align="center">
